...
namespace Eah.WorkSafety.WebApp.Back.Controllers
{
    ...
    public class EmployeeController : ControllerBase
    {
        ...
        public async Task<IActionResult> Search([FromQuery] PaginationFilter filter, string key)
        {
            ...
            var validFilter = new PaginationFilter(filter.PageNumber, filter.PageSize);
            var result = await this.mediator.Send(new GetAllEmployeeByKeyQueryRequest(filter,key));
            var count = await this.mediator.Send(new GetAllEmployeeCountByKeyQueryRequest(key));
            var pagedReponse = PaginationHelper.CreatePagedReponse<EmployeeDto>(result, validFilter, count, uriService, route!);
            return Ok(pagedReponse);
        }
